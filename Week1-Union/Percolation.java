...
public class Percolation { 
     private boolean [][]arr; 
     private final WeightedQuickUnionUF arrqf; 
     private final int len;
     public Percolation(int n){ 
        // create n-by-n grid, with all sites blocked 
       arr=new boolean[n][n];  
       arrqf=new WeightedQuickUnionUF(n*n);
       len=n; 
     } 
     ...
     public void open(int row, int col){    // open site (row, col) if it is not open already 
         ...
         if (arr[row-1][col-1]) return;
         else 
         {            
             arr[row-1][col-1]=true;
             //union with neighbors 
             
             if((row-1>=1)&&(isOpen(row-1,col))) arrqf.union((row-2)*len+col-1,(row-1)*len+col-1);
             if((row+1<=len)&&(isOpen(row+1,col))) arrqf.union((row)*len+col-1,(row-1)*len+col-1);
             if((col-1>=1)&&(isOpen(row,col-1))) arrqf.union((row-1)*len+col-2,(row-1)*len+col-1);
             if((col+1<=len)&&(isOpen(row,col+1))) arrqf.union((row-1)*len+col,(row-1)*len+col-1);
     ...
     public boolean isFull(int row, int col){  // is site (row, col) full? 
         ...
         for(int i=1;i<=len;++i){
             if (isOpen(1,i)&&arrqf.connected(i-1,(row-1)*len+col-1)) return true;
     ...
     {   
         ...
         Percolation newone=new Percolation(10);
         //StdOut.printf("%d\n",newone.len);
         //StdOut.printf("%d\n",newone.len);
         
         StdOut.println("percolate?: "+newone.percolates());
         newone.open(0,6);
         /*
          //newone.isOpen(1,3);
         //newone.open(1,3);
         
         StdOut.println("isFull(r,c) "+newone.isFull(1,2));
         StdOut.println("isFull(r,c) "+newone.isFull(2,3));
         newone.open(3,3);

         newone.open(5,4);
         //StdOut.println("isFull(r,c) "+newone.isFull(3,3));
         //StdOut.println("open? "+newone.open(2,3));
         //StdOut.println("percolate?: "+newone.percolates());
         newone.open(4,4);
         newone.open(6,4);
         newone.open(4,3);
         //StdOut.println("isFull(r,c) "+newone.isFull(6,4));
         StdOut.println("percolate?: "+newone.percolates());
         
         
         StdOut.println("numberOfOpenSites: "+newone.numberOfOpenSites());
         //StdOut.println();
         //StdOut.println("percolate?: "+newone.percolates());
...
public class Percolation {
 ...
 {
  if(!isValid(i,j))
   throw new java.lang.IllegalArgumentException("Invalid indices provided.");
 }
